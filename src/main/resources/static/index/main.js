...
function findDbs() {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            if (status === 1) {
                var queryResult = res.result["queryResult"];
                var data = [];
                if (null != queryResult) {
                    for(var i = 0; i < queryResult.length; i++) {
                        data[i] = queryResult[i]["tables"];
                    }
                }
...
function getFormJsonWithDbPathDeal(dbName) {
    ...
    var oldDbName = dbPathIpt.val();
    dbPathIpt.val(dbName);
    var formJson = $("#consoleForm").serializeObject();
    dbPathIpt.val(oldDbName);
    return formJson;
}
